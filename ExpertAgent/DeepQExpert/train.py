...
def train(env,
    ...
    kwargs_archi["action_size"] = DeepQExpert.get_action_size(env.action_space, filter_action_fun, kwargs_converters)
...
if __name__ == "__main__":
    ...
    env = grid2op.make(dataset=os.path.join(grid2op.get_current_local_dir(), env_name),
                       param=game_param,
    ...
    env.chronics_handler._set_max_iter(7*288)
    try:
        if env.chronics_handler.real_data is not None:
            if env_name == "l2rpn_case14_sandbox":
                env.chronics_handler.real_data.set_filter(lambda x: re.match(".*((03)|(72)|(57))$", x) is not None)
            if env_name == "ai4realnet_small":
                env.chronics_handler.real_data.set_filter(lambda x: re.match(".*(_1)$", x) is not None)
    ...
    tp.initial_epsilon2 = 0.6
    tp.final_epsilon2 = 1./(2*7*288.)
