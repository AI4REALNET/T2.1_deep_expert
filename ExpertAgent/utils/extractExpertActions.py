# python --environment=l2rpn_case14_sandbox --seed=42
import os
import argparse
import logging
import warnings
...
def get_expert_actions(config: ConfigParser, 
    ...
    chronic_indices = list(np.arange(0, num_chronics, 1)) # we can filter here by changing 1->2 and use 2 only for evaluation
    original_action_indices = []
    action_list = []
    action_space_size = 0

    for chronic in tqdm(chronic_indices):
        for timestep in range(env.max_episode_duration()):
            # print(f"Chronic: {chronic:3d}, timestep: {timestep}")
            logger.info(f"Chronic: {chronic:3d}, timestep: {timestep}")
...
if __name__ == "__main__":
    ...
    get_expert_actions(config, env_path, env_name)
